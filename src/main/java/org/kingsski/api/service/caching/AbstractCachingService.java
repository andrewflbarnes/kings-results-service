 ...
 */
public abstract class AbstractCachingService<T> {
    ...
    /**
     * Set the minimum interval (in ms) which must elapse before a call is made to the service to update
     ...
     * the required interval they are retrieved from the {@code service} associated with this class.
     *
    ...
    /**
     * If applicable retrieve the list of items of type {@code T} from the service and update the caching with them.
     * These items of type {@code T} are then returned to the calling method. If an update via the service was not
